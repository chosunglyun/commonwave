...
export default function AdminPage() {
  ...
  const handleAdImageUpload = async (id: string, file: File) => {
    try {
      setStatusMsg({ text: '이미지 업로드 중...', type: 'success' });
      ...
      await updateAd(id, { image_url: data.url });
    } catch (err: any) {
      setStatusMsg({ text: '이미지 업로드 실패: ' + err.message, type: 'error' });
    }
  };
    ...
    <main style={{ minHeight: '100.1vh', background: '#f5f7fa', position: 'relative', overflowY: 'scroll', display: 'block' }}>
      ...
      <div className="container" style={{ paddingTop: '2rem', paddingBottom: '5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800 }}>COMMON WAVE 편집국 (편집자)</h1>
